...
@st.cache_resource
def load_faiss_and_embeddings(faiss_path, model_name="intfloat/multilingual-e5-large", device="cpu"):
    embeddings = HuggingFaceEmbeddings(model_name=model_name, model_kwargs={'device': device})
    db = FAISS.load_local(faiss_path, embeddings, allow_dangerous_deserialization=True)
    return db, embeddings

@st.cache_resource
def load_bart_model(bart_model_name='eugenesiow/bart-paraphrase', device="cpu"):
...
class DocumentRetriever:
    def __init__(self, faiss_path, model_name="intfloat/multilingual-e5-large", device="cpu"):
        ...
        self.model_name = model_name
        self.device = device
        self.db, self.embeddings = load_faiss_and_embeddings(faiss_path, model_name, device)

    def retrieve_documents(self, query, k=2):
        try:
            retrieved_docs = self.db.as_retriever(search_kwargs={'k': k}).get_relevant_documents(query)
            return [doc.page_content[:200] for doc in retrieved_docs]  # Reduced document length
        except Exception as e:
            logging.error(f"Error during document retrieval: {e}")
            raise
...
class BARTGenerator:
    def __init__(self, device="cpu"):
        self.bart_model, self.bart_tokenizer = load_bart_model(device=device)
        self.device = device
    ...
    def generate_response(self, context):
        ...
        inputs = self.bart_tokenizer(answer_part, return_tensors="pt", truncation=True).to(self.device)
        outputs = self.bart_model.generate(inputs["input_ids"], max_length=300, num_beams=2, no_repeat_ngram_size=2, early_stopping=True)  # Reduce beams
        return self.bart_tokenizer.decode(outputs[0], skip_special_tokens=True)

    def extract_answer(self, text):
        question_words = ["what", "how", "why", "when", "who", "where", "is", "are", "can", "should", "do", "does"]
        sentences = text.split('.')
        answer_sentences = [sentence.strip() for sentence in sentences if not sentence.lower().startswith(tuple(question_words)) and not sentence.endswith("?")]
        return '. '.join(answer_sentences).strip() if answer_sentences else None
...
retriever = DocumentRetriever(faiss_path=DB_FAISS_PATH, device=device)
bart_generator = BARTGenerator(device=device)

if query:
    contexts = retriever.retrieve_documents(query, k=2)
    bart_responses = [bart_generator.generate_response(context) for context in contexts] if contexts else ["No relevant document found."]
    
    for i, response in enumerate(bart_responses):
        st.text_area(f"Response {i + 1}", response, height=150)
